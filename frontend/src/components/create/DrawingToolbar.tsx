...
export default function DrawingToolbar({
  ...
  return (
    <div className="h-full flex flex-col p-2 lg:p-4 gap-2 lg:gap-4 border-2 border-border rounded-xl bg-background-secondary">
      {/* Figma-style Toolbar */}
      <div className="bg-background-card rounded-xl px-2 lg:px-3 py-2 flex items-center gap-1 shadow-lg border-2 border-border">
        <button
        ...
        <input
          ...
          className="w-6 h-6 lg:w-8 lg:h-8 rounded cursor-pointer border-2 border-border"
          title="Color"
        />
        ...
        <button
          onClick={onClear}
          className="p-2 lg:p-2.5 rounded-lg text-foreground hover:bg-background-tertiary transition-all"
          title="Clear"
        >
          <svg className="w-4 h-4 lg:w-5 lg:h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <div className="flex-1 flex flex-col space-y-2 lg:space-y-4">
        <div className="bg-background-card rounded-lg border-2 border-border p-2 lg:p-3 space-y-2 lg:space-y-3">
          <textarea
            value={textPrompt}
            onChange={(e) => setTextPrompt(e.target.value)}
            placeholder={placeholder}
            className="w-full bg-transparent text-foreground placeholder-foreground-muted focus:outline-none resize-none text-xs lg:text-sm leading-relaxed min-h-[80px] lg:min-h-[100px]"
            rows={4}
          />
          <button
            onClick={onGenerate}
            disabled={isGenerating || !textPrompt.trim()}
            className="w-full p-2 lg:p-3 rounded-lg bg-accent text-foreground-inverse hover:bg-accent-hover transition-all disabled:bg-background-muted disabled:cursor-not-allowed flex items-center justify-center gap-2 font-medium text-sm lg:text-base"
            title="Generate"
          >
            {isGenerating ? (
              <>
        ...
        <div className="flex-1 flex flex-col space-y-2 lg:space-y-3">
          ...
          <div className="flex flex-wrap gap-1.5 lg:gap-2">
            <button
              ...
              className="px-2 lg:px-3 py-1 lg:py-1.5 text-xs bg-background-tertiary hover:bg-accent hover:text-foreground-inverse text-foreground rounded-full border border-border transition-colors"
            >
              Marvel
            </button>
            <button
